            ...
            <section class="content">
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="container">
                            @if ($role == 'admin')
                                <h1 class="mt-2">Select The Major</h1>
                                <form method="post" action="{{ route('etudiant.formEmploiDuTemps') }}" style="margin: 5% auto; max-width: 260px;"
                                    class="form_customized needs-validation" autocomplete="off" novalidate
                                    enctype="multipart/form-data">
                                    @csrf
                                    @method('post')
                                    <label for="filiere_id" class="form-label" style="text-align: center;">Major</label>
                                    <select required name="filiere_id" id="filiere_id" class="form-control select3 mb-2">
                                        <option value="" disabled selected="selected">Select
                                            The
                                            Major
                                        </option>
                                        @foreach ($filieres as $filiere)
                                            <option @if ($emploieDuTemps && $emploieDuTemps->id == $filiere->id) selected @endif
                                                value="{{ $filiere->id }}">
                                                {{ $filiere->designation }}
                                            </option>
                                        @endforeach
                    ...
                    <div class="mr-auto ml-auto mb-5" style="text-align: center;">
                        @if ($emploieDuTemps)
                            @if ($role == 'tuteur')
                                @foreach ($emploieDuTemps as $id => $data)
                                    <h2 style="font-weight: bolder">
                                        < {{ ucfirst($data['nom']) }} {{ ucfirst($data['prenom']) }}>
                                    </h2>
                                    <a  href="{{ asset('img/emploieDuTemps' . $data['emploieDuTemps']->emploieDuTemps) }}"
                                        target="_blank">
                                        <img class="mb-3" src="{{ @asset('img/emploieDuTemps' . $data['emploieDuTemps']->emploieDuTemps) }}"
                                        style="max-width: 100%; height: auto;" alt="Schedule Not Found">
                                    </a>
                                @endforeach
                            @else
                                <a href="{{ asset('img/emploieDuTemps' . $emploieDuTemps->emploieDuTemps) }}"
                                    target="_blank">
                                    <img src="{{ @asset('img/emploieDuTemps/' . $emploieDuTemps->emploieDuTemps) }}"
                                        alt="Schedule Not Found" style="max-width: 100%; height: auto;">
                                </a>
                            @endif
                        @endif
                    </div>
